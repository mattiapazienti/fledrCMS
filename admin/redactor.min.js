...
(function (c) {
    ...
    c.fn.redactor = function (f) {
        ...
        if (typeof f === "string") {
            this.each(function () {
                var i = c.data(this, "redactor");
                if (typeof i !== "undefined" && c.isFunction(i[f])) {
                    var h = i[f].apply(i, e);
                    if (h !== undefined && h !== i) {
                        g.push(h)
                    }
    ...
    b.fn = c.Redactor.prototype = {
        ...
        init: function (f, e) {
            ...
            if (this.opts.linebreaks === false) {
                if (this.opts.allowedTags !== false && c.inArray("p", this.opts.allowedTags) === "-1") {
                    this.opts.allowedTags.push("p")
                }
                if (this.opts.deniedTags !== false) {
                    var h = c.inArray("p", this.opts.deniedTags);
                    if (h !== "-1") {
                        this.opts.deniedTags.splice(h, h)
                    }
                }
            }
        ...
        syncClean: function (e) {
            ...
            e = e.replace(/<span\s*?>([\w\W]*?)<\/span>/gi, "$1");
            e = e.replace(/<span(.*?)data-redactor="verified"(.*?)>([\w\W]*?)<\/span>/gi, "<span$1$2>$3</span>");
            e = e.replace(/<span(.*?)data-redactor-inlineMethods=""(.*?)>([\w\W]*?)<\/span>/gi, "<span$1$2>$3</span>");
            e = e.replace(/<span\s*?>([\w\W]*?)<\/span>/gi, "$1");
            e = e.replace(/<span\s*?id="selection-marker(.*?)"(.*?)>([\w\W]*?)<\/span>/gi, "");
            e = e.replace(/<span\s*?>([\w\W]*?)<\/span>/gi, "$1");
            e = e.replace(/<span(.*?)data-redactor="verified"(.*?)>([\w\W]*?)<\/span>/gi, "<span$1$2>$3</span>");
            e = e.replace(/<span(.*?)data-redactor-inlineMethods=""(.*?)>([\w\W]*?)<\/span>/gi, "<span$1$2>$3</span>");
        ...
        buildBindKeyboard: function () {
            ...
            this.$editor.on("blur.redactor", c.proxy(function () {
                this.selectall = false
            }, this));
            if (c.isFunction(this.opts.blurCallback)) {
                this.$editor.on("blur.redactor", c.proxy(this.opts.blurCallback, this))
            }
        },
        buildEventDrop: function (i) {
            i = i.originalEvent || i;
            if (window.FormData === undefined) {
                return true
            }
            var h = i.dataTransfer.files.length;
            if (h == 0) {
                return true
            }
            i.preventDefault();
            var g = i.dataTransfer.files[0];
            ...
            this.dragUploadAjax(this.opts.imageUpload, g, true, f, i)
        },
        ...
        buildEventClipboardUpload: function (i) {
        ...
        buildEventKeydown: function (m) {
            if (this.rtePaste) {
                return false
            }
            var f = m.which;
            var i = m.ctrlKey || m.metaKey;
            var g = this.getParent();
            var l = this.getCurrent();
            var n = this.getBlock();
            var j = false;
            this.callback("keydown", m);
            this.imageResizeHide(false);
            if ((g && c(g).get(0).tagName === "PRE") || (l && c(l).get(0).tagName === "PRE")) {
                j = true;
                if (f === this.keyCode.DOWN) {
                    this.insertAfterLastElement(n)
                }
            }
            if (f === this.keyCode.DOWN) {
                if (g && c(g).get(0).tagName === "BLOCKQUOTE") {
                    this.insertAfterLastElement(g)
                }
                if (l && c(l).get(0).tagName === "BLOCKQUOTE") {
                    this.insertAfterLastElement(l)
                }
            }
            if (i && !m.shiftKey) {
                this.shortcuts(m, f)
            }
            if (i && f === 90 && !m.shiftKey && !m.altKey) {
                m.preventDefault();
                ...
                if (f != this.keyCode.LEFT_WIN && !i) {
                    this.selectall = false
                }
            } if (f == this.keyCode.ENTER && !m.shiftKey && !m.ctrlKey && !m.metaKey) {
                if (g.nodeType == 1 && (g.tagName == "TD" || g.tagName == "TH")) {
                    m.preventDefault();
                    this.bufferSet();
                    this.insertNode(document.createElement("br"));
                    this.callback("enter", m);
                    return false
                }
                if (j === true) {
                    return this.buildEventKeydownPre(m, l)
                } else {
                    if (!this.opts.linebreaks) {
                        if (n && this.opts.rBlockTest.test(n.tagName)) {
                            this.bufferSet();
                            setTimeout(c.proxy(function () {
                                var o = this.getBlock();
                                if (o.tagName === "DIV" && !c(o).hasClass("redactor_editor")) {
                    ...
                    if (this.opts.linebreaks) {
                        if (n && this.opts.rBlockTest.test(n.tagName)) {
                            ...
                            return this.buildEventKeydownInsertLineBreak(m)
                        }
                    }
                    if (n.tagName == "BLOCKQUOTE" || n.tagName == "FIGCAPTION") {
                        return this.buildEventKeydownInsertLineBreak(m)
                    }
                }
                this.callback("enter", m)
            } else {
                if (f === this.keyCode.ENTER && (m.ctrlKey || m.shiftKey)) {
                    this.bufferSet();
                    m.preventDefault();
                    this.insertLineBreak()
                }
            } if (f === this.keyCode.TAB && this.opts.shortcuts) {
                return this.buildEventKeydownTab(m, j)
            }
            if (f === this.keyCode.BACKSPACE) {
                this.buildEventKeydownBackspace(l)
            }
        },
        ...
        buildEventKeydownBackspace: function (g) {
            if (typeof g.tagName !== "undefined" && /^(H[1-6])$/i.test(g.tagName)) {
                var e;
                if (this.opts.linebreaks === false) {
            ...
            if (typeof g.nodeValue !== "undefined" && g.nodeValue !== null) {
                var f = c.trim(g.nodeValue.replace(/[^\u0000-\u1C7F]/g, ""));
                if (g.remove && g.nodeType === 3 && g.nodeValue.charCodeAt(0) == 8203 && f == "") {
                    g.remove()
                }
            }
        },
        ...
        buildEventKeyup: function (l) {
            ...
            if (!this.opts.linebreaks && j.nodeType == 3 && (h == false || h.tagName == "BODY")) {
                var i = c("<p>").append(c(j).clone());
                c(j).replaceWith(i);
                var g = c(i).next();
            ...
            if (this.opts.linebreaks === false && (f === this.keyCode.DELETE || f === this.keyCode.BACKSPACE)) {
                return this.formatEmpty(l)
            }
            this.callback("keyup", l);
            this.sync()
        },
        ...
        toolbarObserveScroll: function () {
            var i = c(this.opts.toolbarFixedTarget).scrollTop();
        ...
        airShow: function (j, f) {
            ...
            if (f) {
                ...
                if (this.opts.iframe) {
                    ...
                    i += this.$box.position().left
                }
            } else {
                var g = this.$air.innerWidth();
                i = j.clientX;
                if (c(this.document).width() < (i + g)) {
                    i -= g
                }
                h = j.clientY + 14;
                if (this.opts.iframe) {
        ...
        dropdownBuild: function (f, e) {
            c.each(e, c.proxy(function (i, h) {
                ...
                if (h.name === "separator") {
                    ...
                    g = c('<a href="#" class="' + h.className + " redactor_dropdown_" + i + '">' + h.title + "</a>");
                    g.on("click", c.proxy(function (j) {
                        if (j.preventDefault) {
                            j.preventDefault()
                        }
                        ...
                        if (h.callback) {
                            h.callback.call(this, i, g, h, j)
                        }
                        if (h.exec) {
                            this.execCommand(h.exec, i)
                        }
                        if (h.func) {
                            this[h.func](i)
                        }
        ...
        dropdownShow: function (l, p) {
            if (!this.opts.visual) {
                l.preventDefault();
                return false
            }
            var m = this.$toolbar.find(".redactor_dropdown_box_" + p);
            var f = this.buttonGet(p);
            ...
            var g = c.proxy(function (r) {
                this.dropdownHide(r, m)
            }, this);
            ...
            f.on("click", c.proxy(function (i) {
                ...
                if (e.exec) {
                    ...
                    if (e.func && e.func !== "show") {
                        ...
                        if (e.callback) {
                            e.callback.call(this, h, f, e, i);
                            this.airBindMousemoveHide()
                        } else {
                            if (e.dropdown) {
                                this.dropdownShow(i, h)
                            }
                        }
                    }
                }
        ...
        buttonAddFirst: function (f, g, i, h) {
            if (!this.opts.toolbar) {
                return
            }
            var e = this.buttonBuild(f, {
        ...
        buttonAddAfter: function (l, f, h, j, i) {
            if (!this.opts.toolbar) {
                return
            }
            var e = this.buttonBuild(f, {
        ...
        buttonAddBefore: function (i, f, h, l, j) {
            if (!this.opts.toolbar) {
                return
            }
            var e = this.buttonBuild(f, {
        ...
        buttonActiveObserver: function (h, j) {
            var f = this.getParent();
            this.buttonInactiveAll(j);
            if (h === false && j !== "html") {
                if (c.inArray(j, this.opts.activeButtons) != -1) {
                    this.buttonActiveToggle(j)
                }
                return
            }
            if (f && f.tagName === "A") {
                ...
                c.each(this.opts.activeButtonsAdd, c.proxy(function (e, l) {
                    this.opts.activeButtons.push(l)
                }, this));
                c.extend(this.opts.activeButtonsStates, this.opts.activeButtonsAdd)
            }
            c.each(this.opts.activeButtonsStates, c.proxy(function (e, l) {
        ...
        execCommand: function (h, g, p) {
            ...
            if (h === "inserthtml") {
                this.insertHtml(g, p);
                this.callback("execCommand", h, g);
                return
            }
            if (this.currentOrParentIs("PRE") && !this.opts.formattingPre) {
                return false
            }
            if (h === "insertunorderedlist" || h === "insertorderedlist") {
                this.bufferSet();
                var q = this.getParent();
                var l = c(q).closest("ol, ul");
                var j = false;
                if (l.length) {
                    j = true;
                    var n = l[0].tagName;
                    if ((h === "insertunorderedlist" && n === "OL") || (h === "insertorderedlist" && n === "UL")) {
                        j = false
                    }
                }
                this.selectionSave();
                if (j) {
                    var f = this.getNodes();
                    var e = this.getBlocks(f);
                    if (typeof f[0] != "undefined" && f.length > 1 && f[0].nodeType == 3) {
                        e.unshift(this.getBlock())
                    }
                    var i = "",
                        r = "";
                    c.each(e, c.proxy(function (v, w) {
                        if (w.tagName == "LI") {
                            var u = c(w);
                            var t = u.clone();
                            t.find("ul", "ol").remove();
                            if (this.opts.linebreaks === false) {
                                i += this.outerHtml(c("<p>").append(t.contents()))
                            } else {
                                i += t.html() + "<br>"
                            } if (v == 0) {
                    ...
                    html = this.$editor.html().replace(r, "</" + n + ">" + i + "<" + n + ">");
                    this.$editor.html(html);
                    this.$editor.find(n + ":empty").remove()
                } else {
                    this.document.execCommand(h);
                    var q = this.getParent();
                    var l = c(q).closest("ol, ul");
                    if (l.length) {
                        ...
                        var m = l.parent();
                        if (this.isParentRedactor(m) && this.nodeTestBlocks(m[0])) {
            ...
            if (h === "unlink") {
                this.bufferSet();
                var o = this.currentOrParentIs("A");
                if (o) {
                    c(o).replaceWith(c(o).text());
                    this.sync();
                    this.callback("execCommand", h, g);
                    return
                }
        ...
        indentingStart: function (h) {
            ...
            if (h === "indent") {
                ...
                if (i && i.tagName == "LI") {
                    ...
                    var j = c(n).closest("ol, ul");
                    var m = j[0].tagName;
                    var f = this.getBlocks();
                    c.each(f, function (r, t) {
                        if (t.tagName == "LI") {
                            var q = c(t).prev();
                            if (q.size() != 0 && q[0].tagName == "LI") {
                                var p = q.children("ul, ol");
                                if (p.size() == 0) {
                                    q.append(c("<" + m + ">").append(t))
                                } else {
                                    p.append(t)
                                }
                            }
                    ...
                    if (i === false && this.opts.linebreaks === true) {
                        this.exec("formatBlock", "blockquote");
                        var o = this.getBlock();
                        var i = c('<div data-tagblock="">').html(c(o).html());
                        c(o).replaceWith(i);
                        var g = this.normalize(c(i).css("margin-left")) + this.opts.indentValue;
                        c(i).css("margin-left", g + "px")
                    } else {
                        var e = this.getBlocks();
                        c.each(e, c.proxy(function (q, r) {
                            var p = false;
                            if (r.tagName === "TD") {
                                return
                            }
                            if (c.inArray(r.tagName, this.opts.alignmentTags) !== -1) {
                                p = c(r)
                            } else {
                                p = c(r).closest(this.opts.alignmentTags.toString().toLowerCase(), this.$editor[0])
                            }
                            var s = this.normalize(p.css("margin-left")) + this.opts.indentValue;
                ...
                if (i && i.tagName == "LI") {
                    ...
                    c.each(e, c.proxy(function (q, r) {
                        ...
                        var s = this.normalize(p.css("margin-left")) - this.opts.indentValue;
                        if (s <= 0) {
                            if (this.opts.linebreaks === true && typeof (p.data("tagblock")) !== "undefined") {
                                p.replaceWith(p.html())
                            } else {
                                p.css("margin-left", "");
                                this.removeEmptyAttr(p, "style")
                            }
                        } else {
                            p.css("margin-left", s + "px")
                        }
        ...
        alignmentSet: function (f, h) {
            ...
            if (!i && this.opts.linebreaks) {
                this.exec("formatBlock", "blockquote");
                var e = this.getBlock();
                var i = c('<div data-tagblock="">').html(c(e).html());
                c(e).replaceWith(i);
                c(i).css("text-align", f);
                this.removeEmptyAttr(i, "style");
                if (f == "" && typeof (c(i).data("tagblock")) !== "undefined") {
                    c(i).replaceWith(c(i).html())
                }
            } else {
                var g = this.getBlocks();
                c.each(g, c.proxy(function (l, m) {
                    var j = false;
                    if (c.inArray(m.tagName, this.opts.alignmentTags) !== -1) {
                        j = c(m)
                    } else {
                        j = c(m).closest(this.opts.alignmentTags.toString().toLowerCase(), this.$editor[0])
                    } if (j) {
                        j.css("text-align", f);
        ...
        cleanParagraphy: function (l) {
            ...
            function h(q, i, p) {
                return l.replace(new RegExp(q, i), p)
            }
            ...
            if (l.search(/<blockquote/gi) !== -1) {
                c.each(l.match(/<blockquote(.*?)>([\w\W]*?)<\/blockquote>/gi), function (p, q) {
                    var r = "";
                    r = q.replace("<p>", "");
                    r = r.replace("</p>", "<br>");
                    l = l.replace(q, r)
        ...
        cleanConvertInlineTags: function (e) {
            var f = "strong";
            ...
            if (!/<span(.*?)data-redactor="verified"(.*?)>([\w\W]*?)<\/span>/gi.test(e)) {
                e = e.replace(/<span(.*?)(?!data-redactor="verified")(.*?)>([\w\W]*?)<\/span>/gi, '<span$1 data-redactor="verified"$2>$3</span>')
            }
            return e
        },
        cleanStripTags: function (g) {
            ...
            g = g.replace(f, function (j, i) {
                if (h === true) {
                    return c.inArray(i.toLowerCase(), e) > "-1" ? j : ""
                } else {
                    return c.inArray(i.toLowerCase(), e) > "-1" ? "" : j
                }
            });
        ...
        cleanUnverified: function () {
            ...
            e.not('[data-redactor="verified"]').filter('[style*="font-size"][style*="line-height"]').css("font-size", "").css("line-height", "");
            e.not('[data-redactor="verified"]').filter('[style*="background-color: transparent;"][style*="line-height"]').css("background-color", "").css("line-height", "");
            e.not('[data-redactor="verified"]').filter('[style*="background-color: transparent;"]').css("background-color", "");
            e.not('[data-redactor="verified"]').css("line-height", "");
        ...
        formatBlocks: function (e) {
            ...
            var f = this.getBlocks();
            this.selectionSave();
            c.each(f, c.proxy(function (g, h) {
                if (h.tagName !== "LI") {
                    this.formatBlock(e, h)
                }
            }, this));
            this.selectionRestore();
            this.sync()
        ...
        formatQuote: function () {
            ...
            if (this.opts.linebreaks === false) {
                ...
                if (j) {
                    c.each(j, c.proxy(function (m, n) {
                        if (n.tagName === "BLOCKQUOTE") {
                            this.formatBlock("p", n, false)
                        } else {
                            if (n.tagName !== "LI") {
                                this.formatBlock("blockquote", n, false)
                            }
                        }
                    }, this))
                }
                ...
                if (i.tagName === "BLOCKQUOTE") {
                    this.selectionSave();
                    c(i).replaceWith(c(i).html() + "<br>");
                    this.selectionRestore()
                } else {
                    var l = this.selectionWrap("blockquote");
                    var g = c(l).html();
                    var f = ["ul", "ol", "table", "tr", "tbody", "thead", "tfoot", "dl"];
                    c.each(f, function (m, n) {
                        g = g.replace(new RegExp("<" + n + "(.*?)>", "gi"), "");
                        g = g.replace(new RegExp("</" + n + ">", "gi"), "")
                    });
                    var e = this.opts.blockLevelElements;
                    e.push("td");
                    c.each(e, function (m, n) {
                        g = g.replace(new RegExp("<" + n + "(.*?)>", "gi"), "");
                        g = g.replace(new RegExp("</" + n + ">", "gi"), "<br>")
                    });
                    c(l).html(g);
                    this.selectionElement(l);
                    var h = c(l).next();
                    if (h.size() != 0 && h[0].tagName === "BR") {
                        h.remove()
                    }
                }
            }
        ...
        inlineMethods: function (h, e, i) {
            ...
            if ((f.collapsed || f.startContainer === f.endContainer) && g && !this.nodeTestBlocks(g)) {
                ...
                var j = this.$editor.find("font");
                c.each(j, c.proxy(function (l, m) {
                    this.inlineSetMethods(h, m, e, i)
                }, this))
            }
            this.selectionRestore();
            this.sync()
        },
        inlineSetMethods: function (i, h, e, j) {
            var g = c(h).parent(),
                f;
            if (g && g[0].tagName === "SPAN" && g[0].attributes.length != 0) {
                f = g;
                c(h).replaceWith(c(h).html())
            } else {
                f = c('<span data-redactor="verified" data-redactor-inlineMethods>').append(c(h).contents());
                c(h).replaceWith(f)
            }
            c(f)[i](e, j);
            return f
        },
        inlineEachNodes: function (i) {
            var f = this.getRange(),
                g = this.getElement(),
                e = this.getNodes(),
                h;
            ...
            c.each(e, c.proxy(function (j, l) {
                if (!h && l.tagName !== "SPAN") {
                    if (l.parentNode.tagName === "SPAN" && !c(l.parentNode).hasClass("redactor_editor")) {
                        l = l.parentNode
                    } else {
                        return
                    }
        ...
        insertHtml: function (g, i) {
            ...
            if (e.contents().length == 1) {
                var j = e.contents()[0].tagName;
                if (j != "P" && j == f.tagName || j == "PRE") {
                    g = e.text();
                    e = c("<div>").append(g)
                }
            }
            if (!this.opts.linebreaks && e.contents().length == 1 && e.contents()[0].nodeType == 3 && (this.getRangeSelectedNodes().length > 2 || (!l || l.tagName == "BODY" && !h || h.tagName == "HTML"))) {
                g = "<p>" + g + "</p>"
            }
            if (e.contents().length > 1 && f || e.contents().is("p, :header, ul, ol, div, table, blockquote, pre, address, section, header, footer, aside, article")) {
                if (this.browser("msie")) {
                    this.document.selection.createRange().pasteHTML(g)
                } else {
        ...
        insertHtmlAdvanced: function (f, j) {
            var l = this.getSelection();
            if (l.getRangeAt && l.rangeCount) {
                var e = l.getRangeAt(0);
                e.deleteContents();
                var g = this.document.createElement("div");
                g.innerHTML = f;
                var m = this.document.createDocumentFragment(),
                    i, h;
                while ((i = g.firstChild)) {
                    h = m.appendChild(i)
                }
                e.insertNode(m);
                if (h) {
        ...
        insertNodeToCaretPositionFromPoint: function (j, i) {
            var g;
            var f = j.clientX,
                m = j.clientY;
            if (this.document.caretPositionFromPoint) {
                var l = this.document.caretPositionFromPoint(f, m);
                g = this.getRange();
                g.setStart(l.offsetNode, l.offset);
                g.collapse(true);
                g.insertNode(i)
            } else {
                if (this.document.caretRangeFromPoint) {
                    g = this.document.caretRangeFromPoint(f, m);
                    g.insertNode(i)
                } else {
        ...
        insertAfterLastElement: function (e) {
            if (this.isEndOfElement()) {
                if (c(c.trim(this.$editor.html())).get(0) != c.trim(e) && this.$editor.contents().last()[0] !== e) {
                    return false
                }
                this.bufferSet();
                if (this.opts.linebreaks === false) {
        ...
        pasteClean: function (g) {
            ...
            g = g.replace(/<p(.*?)class="MsoListParagraphCxSpFirst"([\w\W]*?)<\/p>/gi, "<ul><p$2</p>");
            g = g.replace(/<p(.*?)class="MsoListParagraphCxSpLast"([\w\W]*?)<\/p>/gi, "<p$2</p></ul>");
            ...
            g = g.replace(/<img(.*?)style="(.*?)"(.*?)>/gi, "[img$1$3]");
        ...
        pasteClipboardUploadMozilla: function () {
            ...
            c.each(e, c.proxy(function (h, j) {
                ...
                c.post(this.opts.clipboardUploadUrl, {
                    ...
                    var i = (typeof n === "string" ? c.parseJSON(n) : n);
                    g.attr("src", i.filelink);
                    g.removeAttr("data-mozilla-paste-image");
                    this.sync();
                    this.callback("imageUpload", g, i)
                }, this))
            }, this))
        },
        pasteClipboardUpload: function (i) {
            var g = i.target.result;
            var f = g.split(",");
            var h = f[1];
            var j = f[0].split(";")[0].split(":")[1];
            if (this.opts.clipboardUpload) {
                c.post(this.opts.clipboardUploadUrl, {
                    contentType: j,
                    data: h
                }, c.proxy(function (m) {
                    var l = (typeof m === "string" ? c.parseJSON(m) : m);
                    var e = '<img src="' + l.filelink + '" id="clipboard-image-marker" />';
                    this.execCommand("inserthtml", e, false);
                    var n = c(this.$editor.find("img#clipboard-image-marker"));
                    if (n.length) {
                        n.removeAttr("id")
                    } else {
                        n = false
        ...
        observeTables: function () {
            this.$editor.find("table").on("click", c.proxy(this.tableObserver, this))
        },
        ...
        linkObserver: function (h) {
            ...
            if (this.opts.iframe) {
                ...
                l.top = g.top + (l.top - c(this.document).scrollTop());
                l.left += g.left
            }
            var o = c('<span class="redactor-link-tooltip"></span>');
            var f = i.attr("href");
            if (f.length > 24) {
                f = f.substring(0, 24) + "..."
            }
            var j = c('<a href="' + i.attr("href") + '" target="_blank">' + f + "</a>").on("click", c.proxy(function (p) {
                this.linkObserverTooltipClose(false)
            }, this));
            var m = c('<a href="#">' + this.opts.curLang.edit + "</a>").on("click", c.proxy(function (p) {
                p.preventDefault();
                this.linkShow();
                this.linkObserverTooltipClose(false)
            }, this));
            var n = c('<a href="#">' + this.opts.curLang.unlink + "</a>").on("click", c.proxy(function (p) {
                p.preventDefault();
        ...
        selectionWrap: function (e) {
            ...
            var h = this.getBlock();
            if (h) {
                var i = this.formatChangeTag(h, e);
                this.sync();
                return i
            }
            var g = this.getSelection();
            ...
            i.appendChild(f.extractContents());
            f.insertNode(i);
        ...
        getSelectedNodes: function (j) {
            if (typeof j == "undefined" || j == false) {
                var j = this.getRange()
            }
            if (j && j.collapsed === true) {
                return [this.getCurrent()]
            }
            var f = this.getSelection();
            try {
                var p = f.getRangeAt(0).cloneContents()
            } catch (n) {
                return (false)
            }
            var l = this.document.createElement("span");
            l.appendChild(p);
            window.selnodes = l.childNodes;
            var m = selnodes.length;
            var g = [];
            for (var h = 0, o = m; h < o; h++) {
                g.push(selnodes[h])
            }
            if (g.length == 0) {
                g.push(this.getCurrent())
            }
            return g
        },
        getNodes: function (g, e) {
            if (this.opts.linebreaks) {
                return this.getSelectedNodes(g)
            }
            ...
            c.each(f, c.proxy(function (l, m) {
                if (this.opts.iframe === false && c(m).parents("div.redactor_editor").size() == 0) {
                    return false
                }
                if (typeof e === "undefined") {
                    if (c.trim(m.textContent) != "") {
                        i.push(m)
                    }
                } else {
                    if (m.tagName == e) {
        ...
        tableInsert: function () {
            ...
            this.tableObserver(q);
            this.buttonActiveObserver();
            q.find("span#selection-marker-1").remove();
            q.removeAttr("id");
            this.sync()
        },
        tableObserver: function (f) {
            this.$table = c(f.target || f).closest("table");
            this.$tbody = c(f.target).closest("tbody");
            this.$thead = this.$table.find("thead");
            this.$current_td = c(f.target || this.$table.find("td").first());
            this.$current_tr = c(f.target || this.$table.find("tr").first()).closest("tr")
        },
        ...
        tableDeleteRow: function () {
            ...
            this.bufferSet();
            var e = this.$current_tr.prev().length ? this.$current_tr.prev() : this.$current_tr.next();
            if (e.length) {
                var f = e.children("td").first();
                if (f.length) {
                    f.prepend('<span id="selection-marker-1">' + this.opts.invisibleSpace + "</span>");
                    this.selectionRestore()
                }
            }
        ...
        tableDeleteColumn: function () {
            ...
            this.$table.find("tr").each(c.proxy(function (f, g) {
                var h = e - 1 < 0 ? e + 1 : e - 1;
                if (f === 0) {
                    c(g).find("td").eq(h).prepend('<span id="selection-marker-1">' + this.opts.invisibleSpace + "</span>");
                    this.selectionRestore()
                }
                c(g).find("td").eq(e).remove()
            }, this));
            this.sync()
        },
        tableAddHead: function () {
            ...
            if (this.$table.find("thead").size() !== 0) {
                ...
                e.find("td").html(this.opts.invisibleSpace);
                this.$thead = c("<thead></thead>");
                this.$thead.append(e);
        ...
        tableAddRow: function (e) {
            ...
            this.bufferSet();
            var f = this.$current_tr.clone();
            f.find("td").html(this.opts.invisibleSpace);
            if (e === "after") {
                this.$current_tr.after(f)
            } else {
                this.$current_tr.before(f)
            }
            this.sync()
        },
        tableAddColumn: function (f) {
            if (typeof (this.$current_tr) === "undefined") {
                return false
            }
            this.bufferSet();
            var e = 0;
            this.$current_tr.find("td").each(c.proxy(function (g, h) {
                if (c(h)[0] === this.$current_td[0]) {
                    e = g
                }
            }, this));
            this.$table.find("tr").each(c.proxy(function (g, j) {
                var h = c(j).find("td").eq(e);
                var l = h.clone();
                l.html(this.opts.invisibleSpace);
                f === "after" ? h.after(l) : h.before(l)
            }, this));
            this.sync()
        },
        videoShow: function () {
        ...
        linkShow: function () {
            ...
            var e = c.proxy(function () {
                ...
                c(".redactor_link_text").val(m);
                var o = self.location.href.replace(/\/$/i, "");
                var l = f.replace(o, "");
                if (this.opts.linkProtocol === false) {
                    var n = new RegExp("^(http|ftp|https)://" + self.location.host, "i");
                    l = l.replace(n, "")
                }
                if (f.search("mailto:") === 0) {
                    ...
                    if (l.search(/^#/gi) === 0) {
                        ...
                        c("#redactor_link_anchor").val(l.replace(/^#/gi, ""))
                    } else {
                        c("#redactor_link_url").val(l)
                    }
                } if (i === "_blank") {
        ...
        linkProcess: function () {
            var i = c("#redactor_tab_selected").val();
            var g = "",
                m = "",
                j = "",
                l = "";
            if (i === "1") {
                ...
                m = c("#redactor_link_url_text").val();
                ...
                if (i === "2") {
                    ...
                    m = c("#redactor_link_mailto_text").val()
                } else {
                    if (i === "3") {
                        g = "#" + c("#redactor_link_anchor").val();
                        m = c("#redactor_link_anchor_text").val()
                    }
                }
            }
            m = m.replace(/<|>/g, "");
            this.linkInsert('<a href="' + g + '"' + j + ">" + m + "</a>", c.trim(m), g, l)
        },
        ...
        imageShow: function () {
            ...
            var e = c.proxy(function () {
                if (this.opts.imageGetJson) {
                    c.getJSON(this.opts.imageGetJson, c.proxy(function (l) {
                        var h = {}, j = 0;
                        c.each(l, c.proxy(function (n, o) {
                            if (typeof o.folder !== "undefined") {
                                j++;
                                h[o.folder] = j
                            }
                        }, this));
                        var i = false;
                        c.each(l, c.proxy(function (q, r) {
                            var p = "";
                            if (typeof r.title !== "undefined") {
                                p = r.title
                            }
                            var n = 0;
                            if (!c.isEmptyObject(h) && typeof r.folder !== "undefined") {
                                n = h[r.folder];
                                if (i === false) {
                                    i = ".redactorfolder" + n
                                }
                            }
                            var o = c('<img src="' + r.thumb + '" class="redactorfolder redactorfolder' + n + '" rel="' + r.image + '" title="' + p + '" />');
                            c("#redactor_image_box").append(o);
                            c(o).click(c.proxy(this.imageThumbClick, this))
                        }, this));
                        if (!c.isEmptyObject(h)) {
                            ...
                            var m = function (n) {
                                c(".redactorfolder").hide();
                                c(".redactorfolder" + c(n.target).val()).show()
                            };
                            var g = c('<select id="redactor_image_box_select">');
                            c.each(h, function (o, n) {
                                g.append(c('<option value="' + n + '">' + o + "</option>"))
                            });
        ...
        imageEdit: function (g) {
            ...
            this.modalInit(this.opts.curLang.image, this.opts.modal_image_edit, 380, h)
        },
        ...
        imageSave: function (h) {
            ...
            var m = c("#redactor_form_image_align").val();
            if (m === "left") {
                f.css({
                    "float": "left",
                ...
                if (m === "right") {
                    ...
                    var j = f.closest("#redactor-image-box");
                    if (j.size() != 0) {
                        j.css({
                            "float": "",
                            margin: ""
            ...
            var i = c.trim(c("#redactor_file_link").val());
            if (i !== "") {
                var l = false;
                if (c("#redactor_link_blank").prop("checked")) {
                    l = true
                ...
                if (g.get(0).tagName !== "A") {
                    var e = c('<a href="' + i + '">' + this.outerHtml(h) + "</a>");
                    if (l) {
        ...
        imageResizeHide: function (h) {
            if (h !== false && c(h.target).parent().size() != 0 && c(h.target).parent()[0].id === "redactor-image-box") {
                return false
            }
        ...
        imageResize: function (f) {
            ...
            e.on("click", c.proxy(function (j) {
                if (this.$editor.find("#redactor-image-box").size() != 0) {
                    return false
                }
                var m = false,
                    p, o, l = e.width() / e.height(),
                    n = 20,
                    i = 10;
                var g = this.imageResizeControls(e);
                var h = false;
                g.on("mousedown", function (q) {
                    h = true;
                    q.preventDefault();
                    l = e.width() / e.height();
                    p = Math.round(q.pageX - e.eq(0).offset().left);
                    o = Math.round(q.pageY - e.eq(0).offset().top)
                });
                c(this.document.body).on("mousemove", c.proxy(function (u) {
                    if (h) {
                        var r = Math.round(u.pageX - e.eq(0).offset().left) - p;
                        var q = Math.round(u.pageY - e.eq(0).offset().top) - o;
                        var t = e.height();
                        var v = parseInt(t, 10) + q;
                        var s = Math.round(v * l);
                        if (s > n) {
                            e.width(s);
                            if (s < 100) {
                        ...
                        p = Math.round(u.pageX - e.eq(0).offset().left);
                        o = Math.round(u.pageY - e.eq(0).offset().top);
                        this.sync()
                    }
                }, this)).on("mouseup", function () {
                    h = false
                });
                this.$editor.on("keydown.redactor-image-delete", c.proxy(function (r) {
                    var q = r.which;
                    if (this.keyCode.BACKSPACE == q || this.keyCode.DELETE == q) {
                        this.imageResizeHide(false);
                        this.imageRemove(e)
                    }
                }, this));
        ...
        modalInit: function (l, h, f, m) {
            ...
            var i = c("#redactor_modal");
            if (!i.length) {
                this.$modal = i = c('<div id="redactor_modal" style="display: none;"><div id="redactor_modal_close">&times;</div><header id="redactor_modal_header"></header><div id="redactor_modal_inner"></div></div>');
                c("body").append(this.$modal)
            }
            c("#redactor_modal_close").on("click", c.proxy(this.modalClose, this));
            this.hdlModalClose = c.proxy(function (n) {
                if (n.keyCode === this.keyCode.ESC) {
                    this.modalClose();
                    return false
                }
            }, this);
            ...
            i.find("#redactor_modal_header").html(l);
            if (typeof c.fn.draggable !== "undefined") {
                i.draggable({
                    handle: "#redactor_modal_header"
                });
                i.find("#redactor_modal_header").css("cursor", "move")
            }
            var j = c("#redactor_tabs");
            if (j.length) {
                var g = this;
                j.find("a").each(function (n, o) {
                    n++;
                    c(o).on("click", function (q) {
                        q.preventDefault();
                        j.find("a").removeClass("redactor_tabs_act");
                        c(this).addClass("redactor_tabs_act");
                        c(".redactor_tab").hide();
                        c("#redactor_tab" + n).show();
                        c("#redactor_tab_selected").val(n);
                        if (g.isMobile() === false) {
                            var p = i.outerHeight();
                            i.css("margin-top", "-" + (p + 10) / 2 + "px")
                        }
                    })
                })
            }
            i.find(".redactor_btn_modal_close").on("click", c.proxy(this.modalClose, this));
            if (this.opts.autoresize === true) {
            ...
            if (!g) {} else {
                g.onload = c.proxy(function () {
                    if (g.status == 200) {
                        ...
                        this.selectionRestore();
                        var h = "";
                        h = '<img id="image-marker" src="' + j[0] + '" />';
                        if (this.opts.paragraphy) {
                            h = "<p>" + h + "</p>"
                        }
                        this.execCommand("inserthtml", h, false);
                        var i = c(this.$editor.find("img#image-marker"));
                        if (i.length) {
                            i.removeAttr("id")
                        } else {
                            i = false
                        ...
                        this.sync();
                        this.callback("imageUpload", i, false);
                        this.modalClose();
                        this.observeImages()
                    } else {}
                }, this);
        ...
        uploadForm: function (i, h) {
            if (this.uploadOptions.input) {
                var j = "redactorUploadForm" + this.id,
                    e = "redactorUploadFile" + this.id;
                this.form = c('<form  action="' + this.uploadOptions.url + '" method="POST" target="' + h + '" name="' + j + '" id="' + j + '" enctype="multipart/form-data" />');
                if (this.opts.uploadFields !== false && typeof this.opts.uploadFields === "object") {
                    c.each(this.opts.uploadFields, c.proxy(function (m, f) {
                        ...
                        c(this.form).append(n)
                    }, this))
                }
                var g = this.uploadOptions.input;
                var l = c(g).clone();
                c(g).attr("id", e).before(l).appendTo(this.form);
                c(this.form).css("position", "absolute").css("top", "-2000px").css("left", "-2000px").appendTo("body");
                this.form.submit()
            } else {
                i.attr("target", h).attr("method", "POST").attr("enctype", "multipart/form-data").attr("action", this.uploadOptions.url);
                this.element.submit()
            }
        },
        ...
        draguploadInit: function (f, e) {
            ...
            this.dropareabox.get(0).ondrop = c.proxy(function (g) {
                ...
                this.dragUploadAjax(this.draguploadOptions.url, g.dataTransfer.files[0], false)
            }, this)
        },
        dragUploadAjax: function (h, j, f, g, l) {
            if (!f) {
                var m = c.ajaxSettings.xhr();
                if (m.upload) {
                    m.upload.addEventListener("progress", c.proxy(this.uploadProgress, this), false)
                }
                c.ajaxSetup({
                    xhr: function () {
                        return m
            ...
            i.append("file", j);
            if (this.opts.uploadFields !== false && typeof this.opts.uploadFields === "object") {
                c.each(this.opts.uploadFields, c.proxy(function (n, e) {
            ...
            c.ajax({
                ...
                success: c.proxy(function (o) {
                    ...
                    if (f) {
                        ...
                        var e = c("<img>");
                        e.attr("src", n.filelink).attr("id", "drag-image-marker");
                        this.insertNodeToCaretPositionFromPoint(l, e[0]);
                        var p = c(this.$editor.find("img#drag-image-marker"));
                        if (p.length) {
                            p.removeAttr("id")
                        } else {
                            p = false
                        ...
                        this.observeImages();
                        if (p) {
                            this.callback("imageUpload", p, n)
                        }
                        if (typeof n.error !== "undefined") {
                            this.callback("imageUploadError", n)
                        }
                    } else {
                        if (typeof n.error == "undefined") {
                            this.draguploadOptions.success(n)
                        } else {
                            this.draguploadOptions.error(this, n);
        ...
        browser: function (f) {
            ...
            var e = /(chrome)[ \/]([\w.]+)/.exec(g) || /(webkit)[ \/]([\w.]+)/.exec(g) || /(opera)(?:.*version|)[ \/]([\w.]+)/.exec(g) || /(msie) ([\w.]+)/.exec(g) || g.indexOf("compatible") < 0 && /(mozilla)(?:.*? rv:([\w.]+)|)/.exec(g) || [];
            if (f == "version") {
                return e[2]
            }
            if (f == "webkit") {
        ...
        while (l--) {
            ...
            if (h.nodeType === 3) {
                ...
                if (u && p && (p.match(s) || p.match(q))) {
                    ...
                    p = p.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(s, '$1<a href="' + x + '$2">' + f + "</a>$3").replace(q, '$1<a href="$2">' + f + "</a>$5");
                    c(h).after(p).remove()
                }
            } else {
